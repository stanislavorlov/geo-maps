...
MODE_PROFILES: dict[TravelMode, dict[str, float]] = {
    TravelMode.DRIVING: {
        "motorway": 70.0,
        "motorway_link": 45.0,
        "trunk": 50.0,
...
def get_effective_speed(
    ...
    if access in ("private", "no"):
        ...
        if mode == TravelMode.DRIVING and str(tags.get("motor_vehicle", "")).lower() not in ("yes", "permissive", "destination") and str(tags.get("motorcar", "")).lower() not in ("yes", "permissive", "destination"):
            return None
        if mode == TravelMode.CYCLING and str(tags.get("bicycle", "")).lower() not in ("yes", "permissive", "designated", "destination"):
            return None
        if mode == TravelMode.WALKING and str(tags.get("foot", "")).lower() not in ("yes", "permissive", "designated", "destination"):
            return None

    # Check explicit mode bans
    if mode == TravelMode.DRIVING:
        if str(tags.get("motor_vehicle", "")).lower() in ("no", "private") or str(tags.get("motorcar", "")).lower() in ("no", "private"):
            return None
        ...
        if mode == TravelMode.DRIVING:
            ...
            if oneway in ("yes", "1", "true") or is_default_oneway:
                ...
                has_contraflow = (
                    ...
                    any(c in ("opposite", "opposite_lane", "opposite_track") for c in (cycleway_left, cycleway_right, cycleway_both))
                )
    ...
    if mode == TravelMode.CYCLING:
        ...
        if road_type == "cycleway":
            base_speed = 12.0
        elif cycleway in ("lane", "track", "separate", "opposite", "opposite_lane", "opposite_track", "share_busway") or bicycle in ("designated", "yes"):
    ...
    if surface in ("cobblestone", "sett", "unpaved", "gravel", "fine_gravel", "compacted", "ground", "dirt", "earth", "grass", "wood"):
